var Renderer={
    ...
    _actionLock:false,// true during eat/heal/clean/shower animations
...
@keyframes spriteWalk{0%{transform:translateY(0) rotate(-1deg)}50%{transform:translateY(-3px) rotate(1deg)}}\
@keyframes spriteEat{0%,100%{transform:rotate(0)}25%{transform:rotate(6deg) translateY(3px)}75%{transform:rotate(-3deg)}}\
@keyframes spriteSleep{0%,100%{transform:scale(1)}50%{transform:scale(.97) translateY(2px)}}\
@keyframes spriteHappy{0%,100%{transform:translateY(0)}25%{transform:translateY(-10px) rotate(-3deg)}75%{transform:translateY(-10px) rotate(3deg)}}\
@keyframes spriteSad{0%,100%{transform:translateY(0)}50%{transform:translateY(2px) scale(.97)}}\
@keyframes spriteSick{0%,100%{transform:translateX(0)}25%{transform:translateX(-2px)}75%{transform:translateX(2px)}}';
        document.head.appendChild(style);
        this._applyAnim('idle');
    },

    /* ═══ SINGLE place that changes animation — via style.animationName ═══ */
    ...
    _applyFlip:function(flipped){
        if(this._curFlip===flipped)return;
        this._curFlip=flipped;
        // We set scaleX on the wrapper to avoid touching the img animation
        this.els.petWrapper.style.transform='translateX(-50%) scaleX('+(flipped?-1:1)+')';
    },
    ...
    updatePetSprite:function(pet){
        ...
        // Mood animation (only if not in action lock and not walking)
    ...
    /* ═══ MOVEMENT — only touches wrapper position + flip, NEVER animation ═══ */
    tickMovement:function(pet){
        ...
        if(Math.abs(dx)>2){
            ...
            this.currentPetX+=this.walkDir*.3;
    ...
    /* ═══ RESULT DISPLAY — big gauge animation ═══ */
    showGaugeResult:function(label,pct){
        ...
        requestAnimationFrame(function(){d.querySelector('.gr-fill').style.width=pct+'%';});
        setTimeout(function(){d.remove();},3000);
    },

    /* ═══ FOOD ANIMATION — 10s ═══ */
    petEatAnimation:function(foodEmoji){
        var self=this;this._actionLock=true;this._applyAnimForce('eating');
        // Big ingredient pulsing
        var big=document.createElement('div');big.className='big-food-anim';big.textContent=foodEmoji||'🍽️';
        this.els.scene.appendChild(big);
        // Small ingredients flying to pet
        var feedLoop=setInterval(function(){
            var m=document.createElement('div');m.className='food-fly';m.textContent=foodEmoji||'🌾';
            m.style.left='50%';m.style.top='30%';
            self.els.sceneItems.appendChild(m);
            setTimeout(function(){m.remove();},1200);
            // Miam bubble
            var b=document.createElement('div');b.className='float-item';b.textContent='Miam!';
            b.style.left=(self.currentPetX-5+Math.random()*10)+'%';b.style.top='42%';
            b.style.fontSize='18px';b.style.fontWeight='800';b.style.color='#f0c040';
            self.els.sceneItems.appendChild(b);setTimeout(function(){b.remove();},1500);
        },1200);
        setTimeout(function(){clearInterval(feedLoop);big.remove();self._actionLock=false;self._applyAnimForce('idle');},10000);
    },

    _applyAnimForce:function(name){
        this._curAnim=name;
        var s=this.els.petSprite;if(!s)return;
    ...
    /* ═══ STUDY ANIMATION — 10s with brains ═══ */
    showStudyAnimation:function(){
        ...
        var brainLoop=setInterval(function(){
            ...
            b.style.left=(self.currentPetX-8+Math.random()*16)+'%';b.style.bottom='55%';
            self.els.sceneItems.appendChild(b);setTimeout(function(){b.remove();},2000);
        },600);
        setTimeout(function(){clearInterval(brainLoop);book.remove();self._actionLock=false;},10000);
    },

    /* ═══ SHOWER — 10s with showerhead ═══ */
    showHeavyShower:function(){
        ...
        setTimeout(function(){clearInterval(dropLoop);head.remove();self._actionLock=false;},10000);
    },
    ...
    showBigBroom:function(){
        ...
        setTimeout(function(){clearInterval(loop);ov.remove();self._actionLock=false;},10000);
    },

    /* ═══ SYRINGE — 10s toward pet ═══ */
    showBigSyringe:function(){
        ...
        this.els.scene.appendChild(ov);
        var loop=setInterval(function(){
            var s=document.createElement('div');s.className='big-syringe';s.textContent='💉';s.style.fontSize='80px';s.style.left='50%';s.style.top='30%';
            self.els.sceneItems.appendChild(s);setTimeout(function(){s.remove();},1500);
        },1600);
        setTimeout(function(){clearInterval(loop);ov.remove();self._actionLock=false;},10000);
    },

    /* ═══ HEN VISIT — 1 min, big hearts ═══ */
    showHenVisit:function(h,sz){
        ...
        setTimeout(function(){clearInterval(loop);w.classList.add('hidden');},60000);
    },

    petHappyAnimation:function(){this._applyAnimForce('happy');var s=this;setTimeout(function(){s._applyAnimForce('idle');},1200);},
