var Renderer={
    ...
    /* ════════════════════════════════════════════════════
       CRITICAL: updatePetSprite — NEVER reset className
       Only use classList.add/remove for individual classes
       ════════════════════════════════════════════════════ */
    updatePetSprite:function(pet){
        ...
        // 4. Change animation ONLY if different (prevents restart)
        if(this._lastAnim!==targetAnim){
            // Remove old anim
            if(this._lastAnim) p.classList.remove(this._lastAnim);
            // Add new anim
    ...
    tickMovement:function(pet){
        ...
        if(Math.abs(dx)>2){
            ...
            if(this._lastAnim!=='walking'){
                if(this._lastAnim)this.els.pet.classList.remove(this._lastAnim);
    ...
    petEatAnimation:function(){var p=this.els.pet;if(this._lastAnim)p.classList.remove(this._lastAnim);p.classList.add('eating');this._lastAnim='eating';var s=this;setTimeout(function(){p.classList.remove('eating');p.classList.add('idle');s._lastAnim='idle';},2000);},
    petHappyAnimation:function(){var p=this.els.pet;if(this._lastAnim)p.classList.remove(this._lastAnim);p.classList.add('happy');this._lastAnim='happy';var s=this;setTimeout(function(){p.classList.remove('happy');p.classList.add('idle');s._lastAnim='idle';},1200);},
    ...
    renderStatsDetail:function(pet){
        ...
        html+='<div class="stats-section-title">Infos</div><div class="stats-info-grid"><div class="stats-info-card"><div class="label">Stade</div><div class="value">'+stage.emoji+'</div></div><div class="stats-info-card"><div class="label">Âge</div><div class="value">'+age.days+'j</div></div><div class="stats-info-card"><div class="label">XP</div><div class="value">'+pet.experience+'</div></div><div class="stats-info-card"><div class="label">Évolution</div><div class="value">'+(evo!==null?Math.ceil(evo)+'h':'🏆')+'</div></div></div>';
        return html;
    }
};
